...
void MainWindow::paintEvent(QPaintEvent*)
{
    ...
    if (gamemode == started) {
        ...
        if (player == playerwhite) {
            painter.setPen(QColor(47, 47, 47));
        } else {
            painter.setPen(QColor(91, 165, 133));
        }
...
void MainWindow::mouseMoveEvent(QMouseEvent* event) // 鼠标移动事件
{
    ...
    int x = event->position().rx(); // 鼠标移动时的坐标值
    ...
    if (gametype == EVE) {
            ...
            && y >= EDGE + BLOCK_SIZE && y <= (COL)*BLOCK_SIZE + EDGE) // 保证棋子落在棋盘内部，在外部的无效
        {
            ...
            if (board[chessx][chessy] == 0 // 判定位置是否为空
                ...
                qDebug("release: %d %d , t: %d %d", chessx, chessy, event->position().x(), event->position().y());
...
void MainWindow::writeCSV()
{
    ...
    //    int tempTime=globalTimer->elapsed();
    //    QString title5 = QString("%1,").arg(QString(tempTime));
    ...
    if (goData.open(QIODevice::ReadWrite | QIODevice::Append)) {
        ...
        //     goData.write(title5.toStdString().c_str());
